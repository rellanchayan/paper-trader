"""
blend_autopilot.py — entry point for the BLEND strategy (75% hand-picked stocks
+ 25% ETF ballast), built to fit a Robinhood-style agentic investment account.

Wires Alpaca's price/quote data into the pure planner in blend_engine.py,
enforces the $5,000/day invest cap and the no-day-trade rule (with help from
constitution.py at submit time), and submits sell-before-buy.

Usage:
    python3 code/blend_autopilot.py --dry-run     # plan only, no orders
    python3 code/blend_autopilot.py --execute     # submit paper orders
"""
...
RUNS = STATE / "blend_runs"
STATE_FILE = STATE / "blend_state.json"
CONFIG_FILE = STATE / "blend_config.json"
